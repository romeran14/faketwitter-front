...
const InputTweetsSimplified = ({loginState, twMother}) => {
   ...
   const allUsersToSuggest = useMemo(() => {
    return loginState.allUsers.map(
      item => {
...
addFocus()
    ...
    const uploadHandler = () => {
      ...
      const uploadComment = async (fileName = null, retweet = null) => {
        ...
        Axios.post(`${SERVER_URL}upload/uploadcomment`, {
          ...
          console.log(response,'mensaje de req comentario ',response.data.results[1].insertId);
        Axios.put(`${SERVER_URL}upload/tweetbyid/${response.data.results[1].insertId}`).then(
            (req) => {
              setTextTweet('')
              console.log('req',req)
              dispatchFeed({ type: "NEW_COMMENT", payload: req, message : response.data.message });
            }
          );
        });
      };
  ...
  return (
    <Stack className='inputTweetsView' direction={"row"} paddingX={"4"} paddingY={"2"} spacing={4}>
    ...
    <MentionsInput
          ...
          maxLength="140"
        >
          <Mention  data={allUsersToSuggest} 
